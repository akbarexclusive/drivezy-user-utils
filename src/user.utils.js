...
const env = {
    ROUTE_URL: '',
    LOGIN_CHECK_END_POINT: 'getUserSessionDetails'
}

export function InitializeUserUtil(payload) {
    const properties = Object.keys(env);

    for (let key in payload) {
        if (properties.indexOf(key) == -1 || typeof payload[i] == 'undefined') {
            continue;
        }
        env[key] = payload[key];
    }
...
export const LoginCheck = async () => {
    const result = await Get({ urlPrefix: env.ROUTE_URL, url: env.LOGIN_CHECK_END_POINT });
    let loggedUser = {};
    if (result.success) {
